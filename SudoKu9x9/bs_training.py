...
parser.add_argument('--dropout', default=False, type=bool, help='whether dropout')
...
def train(net, train_set, test_set, opt):
    ...
    optim = optimizer.Adam(net.parameters(), lr=adam_lr, weight_decay=opt.weight_decay)
    ...
    for epoch in range(num_epochs):
        ...
        for batch_idx, sample in enumerate(train_dataloader):
            ...
            logits = net(x, dropout=opt.dropout)
            probs = softmax(logits.reshape(N, M1, M2, -1))
            _, preds = torch.max(logits, dim=-1)

            x, y = sample_search(probs, threshold=0.9)
        ...
        # eval for each epoch
        acc, board_acc = evaluate(net, train_dataloader)
        acc = 100*float(acc)
        board_acc = 100*float(board_acc)
        print('\n \t Train | Epoch %3d Acc@1: %2f%% Board_acc: %.2f%%' % (epoch, acc, board_acc))
...
if __name__ == "__main__":
    ...
    net.cuda()
    eval_dataloader = torch.utils.data.DataLoader(train_set, batch_size=opt.batch_size,
                            shuffle=False, num_workers=2)
