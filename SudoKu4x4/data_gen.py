...
def generate_dataset(trainset, testset, n_train=500, n_test=1000):
    X_train, Y_train = [], []
    for _ in range(n_train):
        print(_)
        images = torch.zeros(4, 4, 1, 32, 32)
        board = SudokuMaster.makeBoard()
        Y_train.append(torch.Tensor(board).unsqueeze(dim=0))
    ...
    torch.save([X_train, Y_train], './data/trainset.pt')
    ...
    for _ in range(n_test):
        ...
        images = torch.zeros(4, 4, 1, 32, 32)
        board = SudokuMaster.makeBoard()
        Y_test.append(torch.Tensor(board).unsqueeze(dim=0))
        for (i, cell) in enumerate(board):
            for (j, k) in enumerate(cell):
    ...
    torch.save([X_test, Y_test], './data/testset.pt')


if __name__ == "__main__":
    generate_dataset(trainset, testset)
